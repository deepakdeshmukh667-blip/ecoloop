...
export default function AdminLoginPage() {
  ...
  const { setProfile, setActiveRole } = useApp();
  ...
  const handleVerifyOtp = async (tokenToVerify?: string) => {
    ...
    try {
      ...
      // Step 2: Fetch profile securely from Supabase database to verify role
      ...
      setProfile((prev) => ({
        ...
        full_name: profileData?.full_name || 'Society Administrator',
        role: userRole,
      }));
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setErrorMessage('Network error while resending admin code.');
      setStatus('error');
    }
  };
